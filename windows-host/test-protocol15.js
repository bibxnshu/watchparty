...
app.whenReady().then(() => {
  protocol.handle('watchparty', (request) => {
    ...
    if (range) {
      ...
      console.log('SERVING RANGE:', start, '-', end, 'CHUNKSIZE:', chunksize);
  ...
  win.loadURL('data:text/html,<video id="v" src="watchparty://local/C:/Users/biban/Downloads/your_movie_watchparty.mp4" autoplay controls></video><script>v.onloadedmetadata = () => { console.log("META LOADED: " + v.duration); v.currentTime = 1000; }; v.onseeked = () => console.log("SEEKED TO: " + v.currentTime); </script>');
